...
struct SignUpView: View {
    ...
    var body: some View {
        ZStack {
            ...
            switch step {
            case 1:
                RegisterNicknameView(step: $step,
                                     nickname: $userInfo.nickname)
            case 2:
                RegisterDefaultInfoView(step: $step,
                                        nickname: userInfo.nickname,
                ...
                ProgressView()
                    .onAppear {
                        ...
                        if appleAuthVM.isUnauthorized {
                            ...
                            authApi.appleRegister(idToken: userInfo.idToken, email: userInfo.email, phoneNumber: userInfo.phoneNumber, nickname: userInfo.nickname, gender: genderString, birthday: birthdayArray) { result, errorMessage in
                                guard result else {
                            ...
                            authApi.kakaoRegister(username: userInfo.username, email: userInfo.email, phoneNumber: userInfo.phoneNumber, nickname: userInfo.nickname, gender: genderString, birthday: birthdayArray) { result, errorMessage in
                                guard result else {
