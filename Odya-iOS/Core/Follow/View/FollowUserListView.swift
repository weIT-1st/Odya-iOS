...
struct SearchedUserListView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            (searchedFollowingUser, searchedFollowerUser) = followHubVM.searchFollowUsers(by: nameToSearch)
        }
        .onChange(of: nameToSearch) { newValue in
            (searchedFollowingUser, searchedFollowerUser) = followHubVM.searchFollowUsers(by: newValue)
        }
        .refreshable {
