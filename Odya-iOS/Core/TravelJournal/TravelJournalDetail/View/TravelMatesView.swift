            ...
            ScrollView(.vertical, showsIndicators: false) {
                VStack(spacing: 16) {
                    ForEach(mates) { mate in
                        NavigationLink(
                            destination: UserProfileView(userId: mate.userId ?? -1, nickname: mate.nickname ?? "No Nickname")
                        ) {
...
struct TravelMatesView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        TravelMatesView(
            mates: [TravelMate(userId: 1, nickname: "홍길동aa1", profileUrl: "", isRegistered: true),
