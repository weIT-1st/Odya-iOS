...
class TravelJournalDetailViewModel: ObservableObject {
  ...
  func getJournalDetail(journalId: Int) {
    ...
    func deleteDailyJournal(journalId: Int, dailyJournalId: Int,
                            completion: @escaping (Bool, String) -> Void ) {
        guard let idToken = idToken else {
          return
        }
        ...
        journalProvider.requestPublisher(.deleteContent(token: idToken, journalId: journalId, contentId: dailyJournalId))
          .filterSuccessfulStatusCodes()
          .sink { apiCompletion in
            switch apiCompletion {
            case .finished:
                self.deletingDailyJournalId = self.deletingDailyJournalId.filter{ $0 != journalId }
                self.journalDetail?.dailyJournals = journalDetail.dailyJournals.filter{ $0.dailyJournalId != journalId }
                completion(true, "")
            case .failure(let error):
                self.deletingDailyJournalId = self.deletingDailyJournalId.filter{ $0 != journalId }
              ...
              if apiError.code == -11000 {
                self.appDataManager.refreshToken { success in
                  // token error handling
                  if success {
                      self.deleteDailyJournal(journalId: journalId, dailyJournalId: dailyJournalId) { success, errMsg in
                          completion(success, errMsg)
                      }
                    return
                  }
