...
class MyJournalsViewModel: ObservableObject {
    ...
    private func getBookmarkedJournals(idToken: String) {
        ...
        journalProvider.requestPublisher(.getBookmarkedJournals(token: idToken, size: nil, lastId: self.lastIdOfBookmarkedJournals))
            .filterSuccessfulStatusCodes()
            .sink { completion in
                switch completion {
                case .finished:
    ...
    private func getTaggedJournals(idToken: String) {
        ...
        journalProvider.requestPublisher(.getTaggedJournals(token: idToken, size: nil, lastId: self.lastIdOfTaggedJournals))
            .filterSuccessfulStatusCodes()
            .sink { completion in
                switch completion {
                case .finished:
